...
```console
sudo install -o 10002 -m 0600 /secure/source/id_ed25519 ./id_ed25519
sudo install -o 10002 -m 0644 /secure/source/known_hosts ./known_hosts
...
`GET /readyz` currently proves that PostgreSQL is connected and initial runtime
reconciliation completed. A dormant controller intentionally remains ready, so
readiness does not open an SSH connection and does not validate an unused key.
