import { ApiError, apiRequest, idempotencyHeaders } from "/assets/shared/js/api.js";
...
const playerFrontend = document.body.dataset.playerFrontend || "terminal";
let panelRequest = null;
let transitionToken = 0;
let countdownTimer = null;
// One immediate refresh plus two delayed observations keeps each action bounded
...
async function loadPanel(challengeId, { updateHistory = false, open = true } = {}) {
  ...
  try {
    const query = new URLSearchParams({ frontend: playerFrontend });
    const response = await fetch(`/bff/fragments/challenges/${encodeURIComponent(challengeId)}?${query}`, {
...
async function reconcileTransition(challengeId, token) {
  ...
  for (const delay of transitionRefreshDelays) {
