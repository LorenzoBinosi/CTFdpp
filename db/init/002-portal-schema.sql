...
--
-- Name: challenge_categories; Type: TABLE; Schema: ctfzone; Owner: -
--

CREATE TABLE ctfzone.challenge_categories (
    id integer GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
    name character varying(80) NOT NULL,
    logo_key character varying(16),
    logo_color character varying(7),
    icon_object_id uuid,
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    CONSTRAINT challenge_categories_name_check CHECK (
        btrim(name) <> ''
        AND octet_length(name) <= 80
        AND name !~ '[[:cntrl:]]'
        AND name !~ U&'[\061C\200E\200F\202A-\202E\2066-\2069]'
    ),
    CONSTRAINT challenge_categories_logo_key_check CHECK (
        logo_key IS NULL
        OR logo_key IN ('web', 'pwn', 'crypto', 'rev', 'misc', 'coding', 'forensics')
    ),
    CONSTRAINT challenge_categories_logo_color_check CHECK (
        (logo_key IS NULL AND logo_color IS NULL)
        OR (logo_key IS NOT NULL AND logo_color ~ '^#[0-9a-f]{6}$')
    ),
    CONSTRAINT challenge_categories_id_name_key UNIQUE (id, name)
);

CREATE UNIQUE INDEX challenge_categories_name_normalized_key
    ON ctfzone.challenge_categories (lower(btrim(name)));


-- Retains only request fingerprints and completed public response data. This
-- makes an administrator's retried create request replay-safe without storing
-- flag templates or other request secrets in the idempotency record.
CREATE TABLE ctfzone.admin_create_idempotency (
    ...
    CONSTRAINT admin_create_idempotency_operation_check CHECK (btrim(operation) <> ''),
    CONSTRAINT admin_create_idempotency_key_check CHECK (btrim(idempotency_key) <> ''),
    CONSTRAINT admin_create_idempotency_hash_check CHECK (octet_length(request_sha256) = 32),
    CONSTRAINT admin_create_idempotency_response_check CHECK (jsonb_typeof(response_data) = 'object')
);

CREATE INDEX admin_create_idempotency_resource_idx
    ON ctfzone.admin_create_idempotency (operation, resource_id);
...
CREATE TABLE ctfzone.challenges (
    ...
    challenge_type character varying(32) NOT NULL DEFAULT 'jeopardy',
    exposure character varying(16) NOT NULL DEFAULT 'public',
    type character varying(80),
    ...
    requirements json,
    CONSTRAINT challenges_challenge_type_check CHECK (challenge_type = 'jeopardy'),
    CONSTRAINT challenges_exposure_check CHECK (exposure IN ('public', 'private')),
    CONSTRAINT challenges_max_attempts_check CHECK (max_attempts IS NULL OR max_attempts >= 0),
    CONSTRAINT challenges_connection_info_check CHECK (
        connection_info IS NULL OR octet_length(connection_info) <= 4096
    )
);
...
CREATE TABLE ctfzone.flags (
    ...
    data jsonb NOT NULL DEFAULT '{"case_sensitive":true,"leet_variation":false,"accept_other_users":false}'::jsonb,
    revision bigint NOT NULL DEFAULT 1,
    CONSTRAINT flags_type_check CHECK (type IN ('static', 'regex', 'generated')),
    CONSTRAINT flags_content_not_blank CHECK (btrim(content) <> ''),
    CONSTRAINT flags_data_object_check CHECK (
        jsonb_typeof(data) = 'object'
        AND data ?& ARRAY['case_sensitive','leet_variation','accept_other_users']
        AND data - ARRAY['case_sensitive','leet_variation','accept_other_users'] = '{}'::jsonb
        AND jsonb_typeof(data->'case_sensitive') = 'boolean'
        AND jsonb_typeof(data->'leet_variation') = 'boolean'
        AND jsonb_typeof(data->'accept_other_users') = 'boolean'
    ),
    CONSTRAINT flags_revision_check CHECK (revision > 0),
    CONSTRAINT flags_id_challenge_key UNIQUE (id, challenge_id)
);


-- A generated assignment is created once, at the first private-instance launch.
-- UUID/mask material is retained so later launches reconstruct the same value;
-- only a keyed, case-aware match tag is stored for verification and uniqueness.
CREATE TABLE ctfzone.user_challenge_flags (
    flag_id integer NOT NULL,
    challenge_id integer NOT NULL,
    user_id integer NOT NULL,
    definition_revision bigint NOT NULL,
    match_tag bytea NOT NULL,
    random_token uuid,
    leet_mask bigint,
    leet_position_count smallint,
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    PRIMARY KEY (flag_id, user_id),
    CONSTRAINT user_challenge_flags_revision_check CHECK (definition_revision > 0),
    CONSTRAINT user_challenge_flags_match_tag_length CHECK (octet_length(match_tag) = 32),
    CONSTRAINT user_challenge_flags_leet_mask_check CHECK (
        (leet_mask IS NULL AND leet_position_count IS NULL)
        OR (
            leet_mask IS NOT NULL
            AND leet_position_count IS NOT NULL
            AND leet_mask > 0
            AND leet_position_count BETWEEN 1 AND 62
            AND leet_mask < (1::bigint << leet_position_count)
        )
    ),
    CONSTRAINT user_challenge_flags_challenge_match_key UNIQUE (challenge_id, match_tag)
);


-- Cross-user generated flags are recorded even when policy rejects them.
-- submission_id is an immutable attempt snapshot/reference; it deliberately has
-- no foreign key so later account or submission cleanup cannot erase evidence.
CREATE TABLE ctfzone.flag_sharing_events (
    ...
    team_id_snapshot integer,
    provided_match_tag bytea NOT NULL,
    accepted boolean NOT NULL,
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    CONSTRAINT flag_sharing_events_distinct_users CHECK (submitting_user_id <> source_user_id),
    CONSTRAINT flag_sharing_events_match_tag_length CHECK (octet_length(provided_match_tag) = 32)
);
...
CREATE TABLE ctfzone.pages (
    ...
    page_type character varying(16) NOT NULL,
    system_key character varying(32),
    visibility character varying(16) NOT NULL DEFAULT 'public',
    navigation_order integer NOT NULL DEFAULT 100,
    revision bigint NOT NULL DEFAULT 1,
    created_at timestamp with time zone NOT NULL DEFAULT now(),
    updated_at timestamp with time zone NOT NULL DEFAULT now(),
    CONSTRAINT pages_label_check CHECK (
        octet_length(label) BETWEEN 1 AND 80
        AND label !~ '[[:cntrl:]]'
        AND label !~ U&'[\061C\200E\200F\202A-\202E\2066-\2069]'
    ),
    CONSTRAINT pages_content_check CHECK (
        octet_length(content) <= 262144
    ),
    CONSTRAINT pages_type_check CHECK (page_type IN ('home', 'system', 'custom')),
    CONSTRAINT pages_visibility_check CHECK (visibility IN ('public', 'private', 'invisible')),
    CONSTRAINT pages_navigation_order_check CHECK (navigation_order BETWEEN 0 AND 10000),
    CONSTRAINT pages_revision_check CHECK (revision > 0),
    CONSTRAINT pages_system_identity_check CHECK (
        (page_type = 'home' AND system_key = 'home' AND endpoint = ''
            AND visibility = 'public' AND navigation_order = 0)
        OR (page_type = 'system' AND content = '' AND navigation_order >= 1 AND (
            (system_key = 'challenges' AND endpoint = 'challenges' AND label = 'Challenges')
            OR (system_key = 'scoreboard' AND endpoint = 'scoreboard' AND label = 'Scoreboard')
        ))
        OR (page_type = 'custom' AND system_key IS NULL AND endpoint <> ''
            AND navigation_order >= 1)
    ),
    CONSTRAINT pages_endpoint_check CHECK (
        endpoint = '' OR (
            octet_length(endpoint) BETWEEN 1 AND 128
            AND endpoint = lower(endpoint)
            AND endpoint ~ '^[a-z0-9][a-z0-9_-]*(/[a-z0-9][a-z0-9_-]*)*$'
        )
    )
);
...
--
-- Name: pages pages_endpoint_key; Type: CONSTRAINT; Schema: ctfzone; Owner: -
--

ALTER TABLE ONLY ctfzone.pages
    ADD CONSTRAINT pages_endpoint_key UNIQUE (endpoint);


--
-- Name: pages pages_system_key_key; Type: CONSTRAINT; Schema: ctfzone; Owner: -
--

ALTER TABLE ONLY ctfzone.pages
    ADD CONSTRAINT pages_system_key_key UNIQUE (system_key);
...
CREATE UNIQUE INDEX idx_flags_one_generated_per_challenge
    ON ctfzone.flags USING btree (challenge_id)
    WHERE type = 'generated';

CREATE INDEX idx_user_challenge_flags_challenge_user
    ON ctfzone.user_challenge_flags USING btree (challenge_id, user_id);

CREATE INDEX idx_flag_sharing_events_challenge_date
    ON ctfzone.flag_sharing_events USING btree (challenge_id, created_at DESC);
...
ALTER TABLE ONLY ctfzone.admin_create_idempotency
    ADD CONSTRAINT admin_create_idempotency_actor_user_id_fkey FOREIGN KEY (actor_user_id) REFERENCES ctfzone.users(id) ON DELETE CASCADE;
...
--
-- Name: challenges challenges_category_fkey; Type: FK CONSTRAINT; Schema: ctfzone; Owner: -
--

ALTER TABLE ONLY ctfzone.challenges
    ADD CONSTRAINT challenges_category_fkey FOREIGN KEY (category_id, category) REFERENCES ctfzone.challenge_categories(id, name) ON UPDATE CASCADE ON DELETE RESTRICT;
...
ALTER TABLE ONLY ctfzone.user_challenge_flags
    ADD CONSTRAINT user_challenge_flags_flag_challenge_fkey FOREIGN KEY (flag_id, challenge_id) REFERENCES ctfzone.flags(id, challenge_id) ON DELETE CASCADE;

ALTER TABLE ONLY ctfzone.user_challenge_flags
    ADD CONSTRAINT user_challenge_flags_user_id_fkey FOREIGN KEY (user_id) REFERENCES ctfzone.users(id) ON DELETE RESTRICT;
