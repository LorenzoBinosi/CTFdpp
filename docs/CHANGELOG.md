# CTFd++ 1.0.0 / 2026-07-03

First release of **CTFd++**, a fork of CTFd.

**Rebrand**

- Rename the product to CTFd++ and reset the version to 1.0.0

**Removed**

- Remove the MajorLeagueCyber (MLC) integration entirely (OAuth login, `oauth_id`
  columns, registration mode, admin config, setup wizard step)
- Remove the upstream update check / "new version available" banner and the
  anonymous telemetry to `versioning.ctfd.io`
- Remove the plugin system: the built-in challenge types (standard, dynamic) and
  flag types (static, regex) now live in the core `CTFd.challenges` and
  `CTFd.flags` packages

**Deployment**

- Use Caddy as the default reverse proxy (automatic HTTPS) with a
  `docker-compose.local.yml` override serving plain HTTP on `127.0.0.1:80`
- Reorganize the repository root into `requirements/`, `docs/`, and `deploy/`
