...
pub(super) async fn update_current(
    ...
    let team_id = user
        .team_id
        .ok_or_else(|| ApiError::forbidden("You are not a member of a team"))?;
    let current = load_team(&state, team_id).await?;
    if current.captain_id != Some(user.id) {
...
pub(super) async fn delete_admin(
    ...
    require_team_mode(&state).await?;
    require_admin(&admin)?;
    delete_team(&state, team_id).await
}
...
pub(super) async fn delete_current(
    ...
    if config_string(&state, "team_disbanding").await?.as_deref() == Some("disabled") {
...
pub(super) async fn add_member(
    ...
    let result =
        sqlx::query("UPDATE ctfzone.users SET team_id = $1 WHERE id = $2 AND team_id IS NULL")
            .bind(team_id)
    ...
    if result.rows_affected() == 0 {
        let exists = sqlx::query_scalar::<_, bool>(
            "SELECT EXISTS(SELECT 1 FROM ctfzone.users WHERE id = $1)",
        )
        .bind(request.user_id)
        .fetch_one(&state.database)
        .await
        .map_err(ApiError::database)?;
        return if exists {
            Err(ApiError::conflict("User has already joined a team"))
        } else {
...
async fn delete_team(state: &AppState, team_id: i32) -> Result<Response, ApiError> {
    let mut transaction = state.database.begin().await.map_err(ApiError::database)?;
...
fn invite_code(state: &AppState, team: &TeamRecord) -> Result<String, ApiError> {
    let mut verification_secret = state.auth.secret_key.as_bytes().to_vec();
    ...
    derived_key.update(state.auth.secret_key.as_bytes());
    let mut signature = HmacSha1::new_from_slice(&derived_key.finalize())
        .map_err(|_| ApiError::bad_request("Unable to create an invite code"))?;
...
fn validate_name(value: &str) -> Result<String, ApiError> {
    ...
    if value.is_empty() || value.chars().count() > 128 {
        Err(ApiError::bad_request("Team names must not be empty"))
    } else {
        Ok(value.to_owned())
    }
}
